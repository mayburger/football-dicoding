package com.mayburger.football.mvp.events.next
...
class NextEventsAdapter(var list: List<Events.Event> = arrayListOf(), var ctx: Context, var repo: SoccerRepository, private val listener: (Events.Event) -> Unit) : BaseRecyclerAdapter<NextEventsAdapter.TeamsViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TeamsViewHolder {
        return TeamsViewHolder(LayoutInflater.from(ctx).inflate(R.layout.row_next, parent, false))
    }
    ...
    inner class TeamsViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItem(events: Events.Event, listener: (Events.Event) -> Unit) {
            ...
            favorite.setOnClickListener {
                events.typeEvent = Constants.NEXT_EVENTS
                addEvent(events, ctx)
                unfavorite.visibility = View.VISIBLE
                favorite.visibility = View.GONE
            }
